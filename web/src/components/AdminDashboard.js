...
import { PageFolder, PageFolder2 } from "./PageComponents";
...
const AdminDashboard = () => {
  ...
  const fetchData = async () => {
    ...
    setError("");
    try {
      const [pendingResponse, approvedResponse, patientsResponse] =
        await Promise.all([
    ...
    try {
      ...
      setError(
        err.response?.data?.error ||
          "Failed to toggle doctor status. Please try again."
      );
    } finally {
      setLoading(false);
    }
          ...
          {pendingDoctors.length > 0 && (
            <div className="alert-banner">
              ...
              <span className="alert-text">
                You have {pendingDoctors.length} doctor registration
                {pendingDoctors.length !== 1 ? "s" : ""} pending approval.
              </span>
            </div>
          )}
