...
const AdminPatients = ({
  ...
  const searchTerm = setParentSearchTerm
    ? initialSearchTerm
    : internalSearchTerm;
  const setSearchTerm = setParentSearchTerm || setInternalSearchTerm;
  const statusFilter = setParentStatusFilter
    ? initialStatusFilter
  ...
  const handleTogglePatientStatus = async (patientId, isActive) => {
    if (
      !window.confirm(
        ...
        } this patient? This will cancel any future appointments.`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      setError(
        err.response?.data?.error ||
          "Failed to toggle patient status. Please try again."
      );
  ...
  const filteredPatients = patients.filter((patient) => {
    ...
    const matchesStatus =
      statusFilter === "all" ||
      statusField.toLowerCase() === statusFilter.toLowerCase();
  ...
  const stats = [
    ...
    },
    {
      ...
      value: patients
        .reduce((sum, p) => sum + p.appointmentsCount, 0)
        .toString(),
      subtitle: "All time",
      ...
      <div className="patients-content-area">
                ...
                <tr key={patient.id}>
                  ...
                  <td className="actions-cell">
                    ...
                    <button
                      ...
                      onClick={() =>
                        handleTogglePatientStatus(patient.id, patient.isActive)
                      }
                      disabled={loading}
                    >
                      {patient.isActive ? "Deactivate" : "Reactivate"}
        ...
        {filteredPatients.length === 0 && (
          <div className="empty-state">
            <img
              src="/assets/Admin-assets/Total-Patients.svg"
              alt="no patients"
      ...
      <header className="admin-header">
        ...
        <div className="header-right">
          ...
          <button className="logout-btn" onClick={handleLogout}>
            <img
              src="/assets/Admin-assets/Logout.svg"
              alt="logout"
